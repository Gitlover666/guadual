...
当a=0时，产生应答，a=1时，不产生应答
把IIC_Ack()和IIC_NAck()用一个函数来表示。
***/
// void IIC_Ack(u8 a)
// {
// 	SDA_OUT();
...
/***发送一个字节***/
/***功能:将数据c发送出去，可以是地址，也可以是数据，发送完后等待应答，
并对此状态位进行操作。(不应答或非应答都使ack=0),发送数据正常，ack=1;
ack=0表示被控器无应答或顺坏。
***/
...
//    }
// 	delay_us(2);
// 	IIC_SDA=1;    //8位发送完毕后释放总线，准备接收应答位
// 	delay_us(2);
// 	IIC_SCL=1;
// if(IIC_SDA==1)  IIC_ack=0;
// 	 else IIC_ack=1;    //判断是否收到应答信号
// 	 IIC_SCL=0;      //拉低SCL，为下次数据传输做好准备
// 		delay_us(2);
...
// 		delay_us(2);
// 		receive=receive<<1;
// 		if(IIC_SDA==1) receive=receive+1;    //读取SDA，把接收的数据位放入receve中
// 		delay_us(2);
//   }
// 	IIC_SCL=0;      //在此处有何用处？？？(猜测：为下次数据传输做好准备， //钳住IIC总线，准备发送或者接收数据)
// 	delay_us(2);
