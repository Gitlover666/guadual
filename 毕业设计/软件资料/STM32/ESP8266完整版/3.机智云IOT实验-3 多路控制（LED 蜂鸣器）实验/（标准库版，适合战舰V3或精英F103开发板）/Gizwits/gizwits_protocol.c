...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagBeep_OnOff)
    {
        info->event[info->num] = EVENT_BEEP_ONOFF;
        info->num++;
        dataPoints->valueBeep_OnOff = gizDecompressionValue(BEEP_ONOFF_BYTEOFFSET,BEEP_ONOFF_BITOFFSET,BEEP_ONOFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagLED0_OnOff)
    {
        info->event[info->num] = EVENT_LED0_ONOFF;
        info->num++;
        dataPoints->valueLED0_OnOff = gizDecompressionValue(LED0_ONOFF_BYTEOFFSET,LED0_ONOFF_BITOFFSET,LED0_ONOFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagLED1_OnOff)
    {
        info->event[info->num] = EVENT_LED1_ONOFF;
        info->num++;
        dataPoints->valueLED1_OnOff = gizDecompressionValue(LED1_ONOFF_BYTEOFFSET,LED1_ONOFF_BITOFFSET,LED1_ONOFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueBeep_OnOff != cur->valueBeep_OnOff)
    {
        GIZWITS_LOG("valueBeep_OnOff Changed\n");
        ret = 1;
    }
    if(last->valueLED0_OnOff != cur->valueLED0_OnOff)
    {
        GIZWITS_LOG("valueLED0_OnOff Changed\n");
        ret = 1;
    }
    if(last->valueLED1_OnOff != cur->valueLED1_OnOff)
    {
        GIZWITS_LOG("valueLED1_OnOff Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(BEEP_ONOFF_BYTEOFFSET,BEEP_ONOFF_BITOFFSET,BEEP_ONOFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueBeep_OnOff);
    gizCompressValue(LED0_ONOFF_BYTEOFFSET,LED0_ONOFF_BITOFFSET,LED0_ONOFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED0_OnOff);
    gizCompressValue(LED1_ONOFF_BYTEOFFSET,LED1_ONOFF_BITOFFSET,LED1_ONOFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED1_OnOff);
