...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagBEEP)
    {
        ...
        info->num++;
        dataPoints->valueBEEP = gizDecompressionValue(BEEP_BYTEOFFSET,BEEP_BITOFFSET,BEEP_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(BEEP_BYTEOFFSET,BEEP_BITOFFSET,BEEP_LEN,(uint8_t *)devStatusPtr,dataPoints->valueBEEP);
