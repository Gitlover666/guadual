...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagTemp_AlarmScope)
    {
        info->event[info->num] = EVENT_Temp_AlarmScope;
        info->num++;
        dataPoints->valueTemp_AlarmScope = gizX2Y(Temp_AlarmScope_RATIO,  Temp_AlarmScope_ADDITION, issuedData->attrVals.valueTemp_AlarmScope);
    }

    if(0x01 == issuedData->attrFlags.flagShidu_AlarmScope)
    {
        info->event[info->num] = EVENT_Shidu_AlarmScope;
        info->num++;
        dataPoints->valueShidu_AlarmScope = gizX2Y(Shidu_AlarmScope_RATIO,  Shidu_AlarmScope_ADDITION, issuedData->attrVals.valueShidu_AlarmScope);
    }

    if(0x01 == issuedData->attrFlags.flagGQ_AlarmScope)
    {
        info->event[info->num] = EVENT_GQ_AlarmScope;
        info->num++;
        dataPoints->valueGQ_AlarmScope = gizX2Y(GQ_AlarmScope_RATIO,  GQ_AlarmScope_ADDITION, exchangeBytes(issuedData->attrVals.valueGQ_AlarmScope));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueTemp_AlarmScope != cur->valueTemp_AlarmScope)
    {
        GIZWITS_LOG("valueTemp_AlarmScope Changed\n");
        ret = 1;
    }
    if(last->valueShidu_AlarmScope != cur->valueShidu_AlarmScope)
    {
        GIZWITS_LOG("valueShidu_AlarmScope Changed\n");
        ret = 1;
    }
    if(last->valueGQ_AlarmScope != cur->valueGQ_AlarmScope)
    {
        GIZWITS_LOG("valueGQ_AlarmScope Changed\n");
        ret = 1;
    }
    if(last->valueTemp_Q != cur->valueTemp_Q)
    {
        GIZWITS_LOG("valueTemp_Q Changed\n");
        ret = 1;
    }
    if(last->valueSshidu_Q != cur->valueSshidu_Q)
    {
        GIZWITS_LOG("valueSshidu_Q Changed\n");
        ret = 1;
    }
    ...
    if(last->valueGQ != cur->valueGQ)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueGQ Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints, devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Temp_Q_BYTEOFFSET,Temp_Q_BITOFFSET,Temp_Q_LEN,(uint8_t *)devStatusPtr,dataPoints->valueTemp_Q);
    gizStandardCompressValue(Sshidu_Q_BYTEOFFSET,Sshidu_Q_BITOFFSET,Sshidu_Q_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSshidu_Q);
    ...
    devStatusPtr->valueTemp_AlarmScope = gizY2X(Temp_AlarmScope_RATIO,  Temp_AlarmScope_ADDITION, dataPoints->valueTemp_AlarmScope);
    devStatusPtr->valueShidu_AlarmScope = gizY2X(Shidu_AlarmScope_RATIO,  Shidu_AlarmScope_ADDITION, dataPoints->valueShidu_AlarmScope);
    ...
    devStatusPtr->valueGQ_AlarmScope = exchangeBytes(gizY2X(GQ_AlarmScope_RATIO,  GQ_AlarmScope_ADDITION, dataPoints->valueGQ_AlarmScope));
    devStatusPtr->valueGQ = exchangeBytes(gizY2X(GQ_RATIO,  GQ_ADDITION, dataPoints->valueGQ));
...
static void gizDevReportPolicy(dataPoint_t *currentData)
{
    ...
    if((0 == (timeNow % (100000))) && (lastRepTime != timeNow))
    {
